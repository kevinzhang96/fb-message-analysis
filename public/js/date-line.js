...
var color = d3.scale.ordinal().range(["#72C39B", "#7FC7C6", "#80A8CE", "#9788CD", "#C280B7", "#DC647E", "#E2705C", "#E29D58", "#DDC05E", "#F2DE8A"]);
...
var svg = d3.select("#count-time-vis").append("svg")
...
$.get("/data", function(data) {
  ...
  var topTen = counts.slice(0, 10).map(function(obj) { return obj.name; });

  // get times for each conversation
  var times = Object.keys(data).map(function(person) {
    return {
      name: person,
  ...
  // get person => months mappings
  var personMonths = Object.keys(times).map(function(index) {
    var months = {};
    times[index].times.map(function(date) {
  ...
  // get list of months
  var months = new Set();
  for (var index in personMonths) {
    Object.keys(personMonths[index].months).map(function(month) { months.add(month); });
  }

  // create sorted array of months
  var sortedMonths = Array.from(months);
  sortedMonths.sort(function(a, b) { return  a - b });
  ...
  sortedMonths.map(function(month) {
    var monthIndex = month.getYear() * 12 + month.getMonth();
    var monthData = {};
    personMonths.map(function(personData) {
      var val = personData.months[monthIndex];
      if (val === undefined) {
        monthData[personData.name] = "";
      } else {
        monthData[personData.name] = val.toString();
      }
    });
